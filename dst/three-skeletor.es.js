import { Vector3, Matrix4, Object3D, ConeBufferGeometry, Mesh, MeshBasicMaterial, AxesHelper, Color, Plane, Math as Math$1 } from 'three';
...
const t1$1 = new Vector3();
const t2$1 = new Vector3();
const t3$1 = new Vector3();
const t4 = new Vector3();

const { DEG2RAD, RAD2DEG } = Math$1;
...
const { DEG2RAD: DEG2RAD$1, RAD2DEG: RAD2DEG$1 } = Math$1;
...
class IKBallConstraint {
	...
	_apply(joint) {
		...
		const direction = new Vector3().copy(joint._getDirection());
		const parentDirection = joint
			._localToWorldDirection(new Vector3().copy(Z_AXIS$1))
			.normalize();
...
export { BoneHelper, IK, IKBallConstraint, IKChain, IKHelper, IKHingeConstraint, IKJoint, getCentroid, getWorldDistance, getWorldPosition, setQuaternionFromDirection, transformPoint };
